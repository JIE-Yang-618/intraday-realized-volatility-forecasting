...
class LSTM(BaseForecaster):
    """LSTM forecaster (demo sequence over lagged RV)."""
    name = "LSTM"
    ...
    def __init__(self, hidden_size: int = 32, num_layers: int = 1, dropout: float = 0.0, lr: float = 1e-3, epochs: int = 10, batch_size: int = 128, seq_len: int = 20) -> None:
        self._m = RNNForecaster(cell="lstm", hidden=hidden_size, num_layers=num_layers, dropout=dropout, lr=lr, epochs=epochs, batch_size=batch_size, seq_len=seq_len)
